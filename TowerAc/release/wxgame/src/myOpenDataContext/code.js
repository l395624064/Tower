...
*————请在laya.init()前，填写MiniAdpter.init(true,true);
*————编译后，需要将game.js文件修改为index.js
*————无需激活资源版本控制
*————需要使用wx_tool类
*————无法使用laya.loader.load预载图集资源
*————laya.stage已经在初始化时绘制在sharedCanvas上,获取信息直接在laya.stage上显示即可
*@author ...
*/
//class LayaUISample
var LayaUISample=(function(){
	function LayaUISample(){
		...
		this.mysharedCanvas=null;
		MiniAdpter.init(true,true);
		Laya.init(750,1334);
		console.log("激活子域");
		var onMessage=wx.onMessage;
		onMessage(this.getMsg);
	}
	...
	*@param data
	*/
	__proto.getMsg=function(data){
		console.log("————————接收主域的信息:",data);
		switch (data.type){
			case "self":
				console.log("————个人详细信息————");
				WX_Tool.getUserInfo();
			...
			case "selfscore":
				console.log("————个人分数信息————");
				WX_Tool.getUserCloudStorage(new Array("score"));
				break ;
			case "groupCard":
				console.log("__________群转发名片___________");
				WX_Tool.getGroupCard();
				break ;
			case "friend":
				console.log("________好友________");
				WX_Tool.getFriendCloudStorage(new Array("score"));
				break ;
			case "group":
				console.log("__________群组信息_____________",data._shareTickets);
				WX_Tool.getGroupCloudStorage(data._shareTickets);
				break ;
			case "close":
				console.log("————注销排行榜信息————");
				WX_Tool.clearItemArr();
				break ;
			case "initRank":
				console.log("—————初始化Rankview——————");
				WX_Tool.initStageNum(data);
				break ;
			case "selfItem":
				console.log("—————创建个人信息页—————");
				WX_Tool.creatSingleItem(WX_Tool.friendMsg,data);
				break ;
			case "selfAndside":
				console.log("—————创建selfAndside信息页——————");
				WX_Tool.creatSingleItem(WX_Tool.friendMsg,data,true);
				break ;
			case "friend_nextPage":
				console.log("—————下一页—————");
				WX_Tool.creatListPage(WX_Tool.friendMsg,"nextPage");
				break ;
			case "friend_nextPage":
				console.log("—————上一页—————");
				WX_Tool.creatListPage(WX_Tool.friendMsg,"prvePage");
				break ;
			case "group_nextPage":
				WX_Tool.creatListPage(WX_Tool.groupMsg,"nextPage");
				break ;
			case "group_prvePage":
				WX_Tool.creatListPage(WX_Tool.groupMsg,"prvePage");
				break ;
			default :
			}
	}
...
*/
//class WX_Tool
var WX_Tool=(function(){
	//微信群信息——群排行榜调用
	function WX_Tool(){}
	__class(WX_Tool,'WX_Tool');
	WX_Tool.login=function(){
		var log=wx.login;
		log({
	...
	WX_Tool.authorize=function(_scope){
		var getAuth=wx.authorize;
		getAuth({
			scope:_scope,
			fail:function (res){
	...
	WX_Tool.getSetting=function(_scope){
		var getSet=wx.getSetting;
		getSet({
			success:function (res){
	...
	WX_Tool.getUserInfo=function(_openId){
		(_openId===void 0)&& (_openId=["selfOpenId"]);
		var getMsg=wx.getUserInfo;
		getMsg({
			openIdList:_openId,
			lang:'zh_CN',
			success:function (res){
	...
	WX_Tool.vibrateLong=function(){
		var short=wx.vibrateLong;
		short({
			success:function (){
			},
			fail:function (){
	...
	WX_Tool.vibrateShort=function(){
		var short=wx.vibrateShort;
		short({
			success:function (){
			},
			fail:function (){
	...
	WX_Tool.initStageNum=function(_obj){
		...
		WX_Tool.rankPanel.hScrollBarSkin="";
		WX_Tool.rankPanel.vScrollBarSkin="";
	...
	WX_Tool.sortArr=function(_arr){
		console.log("_arr排序前:",_arr);
		for (var j=0;j < _arr.length;j++){
			if (_arr[j]['KVDataList'].length<=0){
				_arr[j]['KVDataList'][0]={key:"score",value:"0"};
			}
		};
		var resetArr=WX_Tool.Copy(_arr);
		var afterArr=WX_Tool.Sort(resetArr);
		console.log("_arr排序后:",afterArr);
	...
	WX_Tool.Copy=function(src){
		var des=[];
		var obj1;
		for(var $each_obj1 in src){
			obj1=src[$each_obj1];
			des.push(obj1);
		}
		return des;
	...
	WX_Tool.Min=function(arr){
		var min=-65534;var minObject;
	...
	WX_Tool.creatListPage=function(_myArr,_control){
		(_control===void 0)&& (_control="");
		var maxpage=Math.ceil(_myArr.length / 6);
		...
		var myArr=_myArr.slice(WX_Tool.pageNum *6,WX_Tool.pageNum *6+7);
		WX_Tool.creatItem(myArr);
	}

	WX_Tool.creatItem=function(_arr){
		var colorArr=["0x001122","0x111111"];
		var rectW=WX_Tool.rankviewMsg.soldW;
		var rectH=WX_Tool.rankviewMsg.soldH;
		var repairH=WX_Tool.rankviewMsg.pix;
		var icoW=WX_Tool.rankviewMsg.icoW;
		var icoH=WX_Tool.rankviewMsg.icoH;
		var fSize=WX_Tool.rankviewMsg.fontsize;
		var scoreX=WX_Tool.rankviewMsg.scoreX;
		var nameX=WX_Tool.rankviewMsg.nameX;
		var rankX=WX_Tool.rankviewMsg.rankX;
		var rankY=WX_Tool.rankviewMsg.rankY;
		console.log("WX_Tool.rankviewMsg——————",WX_Tool.rankviewMsg);
		...
		for (var i=0;i < _arr.length;i++){
			...
			itemSp.pos(rankX,rankY+i *100+i *repairH);
			rankNum=new Text();
			rankNum.text=_obj['rankNum'];
			...
			itemSp.addChild(name_txt);
			WX_Tool.itemArr.push(itemSp);
			Laya.stage.addChild(itemSp);
		}
		console.log("ad1.0");
	}

	WX_Tool.creatSingleItem=function(_arr,_data,_ifside){
		(_ifside===void 0)&& (_ifside=false);
		var userName=WX_Tool.rankviewMsg["username"];
		var sideArr=[];
		console.log("acs1.2",userName,"sideArr:",sideArr);
		if (_ifside){
		...
		var rectW=_data['rectW'];
		var rectH=_data['rectH'];
		var repairW=_data['repairW'];
		var rankNum;
		var ico;
		var score_txt;
		var name_txt;
		...
		for (var i=0;i < sideArr.length;i++){
			var _obj=_data['itemArr'][i];
			itemSp=new Sprite();
			itemSp.size(rectW,rectH);
			itemSp.pos(0,i *100+i *repairW);
			rankNum=new Text();
			rankNum.text=sideArr[i]['rankNum'];
	...
	WX_Tool.findmyselfSid=function(_arr,_username,_ifsingl){
		(_ifsingl===void 0)&& (_ifsingl=true);
		var _sideArr=[];
		for (var i=0;i < _arr.length;i++){
	...
	WX_Tool.onClick=function(){
		console.log("可以在子域制作panel!!!——————————");
	}

	WX_Tool.getFriendCloudStorage=function(_keyArr){
		var getMsg=wx.getFriendCloudStorage;
		getMsg({
			keyList:_keyArr,
			success:function (res){
				console.log("getFriendCloudStorage获取成功:",res.data);
				WX_Tool.friendMsg=WX_Tool.sortArr(res.data);
	...
	WX_Tool.clearItemArr=function(){
		for (var i=WX_Tool.itemArr.length-1;i >=0;i--){
			var itm=WX_Tool.itemArr[i];
			itm.removeSelf();
		}
		WX_Tool.itemArr=[];
	}

	WX_Tool.getGroupCloudStorage=function(_shareStr,_keyArr){
		(_keyArr===void 0)&& (_keyArr=["score"]);
		var getMsg=wx.getGroupCloudStorage;
		getMsg({
			shareTicket:_shareStr,
			keyList:_keyArr,
			success:function (res){
				console.log("获取群成员数组:",res.data);
				WX_Tool.groupMsg=WX_Tool.sortArr(res.data);
				WX_Tool.creatListPage(WX_Tool.groupMsg);
			},
			fail:function (res){
				console.log("群资料获取失败：",res);
			}
		});
	}

	WX_Tool.getShareInfo=function(_shareTicket){
		var shareInfo=wx.getShareInfo;
		shareInfo({
			shareTicket:_shareTicket,
			success:function (res){
				console.log("获取转发详细信息成功：",res);
			},
			fail:function (res){
				console.log("获取转发详细信息失败：",res);
			}
		});
	}

	WX_Tool.getUserCloudStorage=function(_keyArr){
		var getUCMsg=wx.getUserCloudStorage;
		getUCMsg({
			keyList:_keyArr,
			success:function (res){
	...
	WX_Tool.request=function(_url,_data,_method,_header){
		(_method===void 0)&& (_method="POST");
		var req=wx.request;
		req({
	...
	WX_Tool.setUserStorage=function(_kv,_access,_opid,_appid,_sigN,_sigM){
		(_access===void 0)&& (_access="");
		(_opid===void 0)&& (_opid="");
		(_appid===void 0)&& (_appid="");
		(_sigN===void 0)&& (_sigN="");
		(_sigM===void 0)&& (_sigM="");
		var setMsg=wx.setUserStorage;
		if (_access==""){
			_access=WX_Tool.loginMsg.access_token
		}
		if (_opid==""){
			_opid=WX_Tool.loginMsg.openid
		}
		if (_appid==""){
			_appid=WX_Tool.loginMsg.appid
		}
		if (_sigN==""){
			_sigN=WX_Tool.loginMsg.signature
		}
		if (_sigM==""){
			_sigM=WX_Tool.loginMsg.sig_method
		}
		setMsg({
	...
	WX_Tool.setUserCloudStorage=function(_keyArr){
		var setMsg=wx.setUserCloudStorage;
		setMsg({
			KVDataList:_keyArr,
			success:function (res){
			},
	...
	WX_Tool.removeUserCloudStorage=function(_keyArr){
		var removeMsg=wx.removeUserCloudStorage;
		removeMsg({
			keyList:_keyArr,
			success:function (){}
		});
	...
	WX_Tool.getGroupCard=function(){
		WX_Tool.clearItemArr();
		WX_Tool.canvasImg.graphics.drawCircle(100,100,100,"#ff8765","#ff0000",10);
		WX_Tool.canvasImg.size(500,500);
		Laya.stage.addChild(WX_Tool.canvasImg);
		WX_Tool.canvasImg.graphics.drawCircle(0,0,30,"#778899","#112233",5);
		var scoretxt=new Text();
		scoretxt.text="哈饕嚏哈";
		scoretxt.size=50;
		scoretxt.color="#009876";
		WX_Tool.canvasImg.addChild(scoretxt);
		var scoreTxt=new Text();
	}

	WX_Tool.canvasToTemFilePath=function(){
		var canvasToTempFilePath=wx.canvasToTempFilePath;
		canvasToTempFilePath({
			...
			destHeight:300,
			success:function (res){
				console.log("本地图片缓存地址Path:",res.tempFilePath);
			},
		});
	}

	WX_Tool.shareAppMessage=function(_canvasUrl){
		var shareMsg=wx.shareAppMessage;
		shareMsg({
			title:'这个是本地缓存截图',
			imageUrl:_canvasUrl,
			success:function (res){
				console.log("获取转发shareTicket：",res.shareTickets);
			}
		});
	}
	...
	WX_Tool.friendMsg=[];
	WX_Tool.groupMsg=[];
	WX_Tool.rankviewMsg=null;
	WX_Tool.pageNum=0;
	WX_Tool.signNum=6;
	__static(WX_Tool,
	['userMsg',function(){return this.userMsg=new Object();},'canvasImg',function(){return this.canvasImg=new Sprite();},'rankPanel',function(){return this.rankPanel=new Panel();}
	]);
	return WX_Tool;
})()
