	...
	__proto.getMsg=function(data){
		...
		switch (data.type){
			...
			case "initRank":
				...
				break ;
			case "selfAndside":
				console.log("—————创建selfAndside类型list——————");
				WX_Tool.initStageNum(data,true);
				break ;
			default :
			}
	}
...
var WX_Tool=(function(){
	...
	WX_Tool.initStageNum=function(_obj,_ifside){
		(_ifside===void 0)&& (_ifside=false);
		WX_Tool.rankviewMsg=_obj;
		Laya.stage.width=_obj.stageW;
		Laya.stage.height=_obj.stageH;
		WX_Tool.ifselfSide=_ifside;
	...
	WX_Tool.creatlist=function(res){
		WX_Tool.clearItemArr();
		for (var j=0;j < res.length;j++){
			if (res[j]['KVDataList'].length<=0){
				res[j]['KVDataList'][0]={key:"score",value:"0"};
			}
		};
		var resetArr=WX_Tool.Copy(res);
		var afterArr=WX_Tool.Sort(resetArr);
		var _fsp=WX_Tool.rankPanel;
		console.log("1111111",res);
		var sp;
		var ico;
		var score_txt;
		var name_txt;
		var rectW=0,rectH=0,repairH=0;
		var icoX,icoY,nameX,nameY,scoreX,scoreY,numberX,numberY,fontsize,itemW,itemH,fontcolor;
		if (WX_Tool.ifselfSide){
			...
			var sideArr=WX_Tool.findmyselfSid(afterArr,userName);
			console.log("222222222222222222222222",userName,sideArr);
			return;
		}
		else{
			rectW:int=500;
		...
		for (var i=0;i < afterArr.length;i++){
			...
			console.log("创建testlist");
			_fsp.addChild(sp);
		}
		console.log("ad0.6");
	}

	WX_Tool.findmyselfSid=function(_arr,_username){
		var _sideArr=[];
	...
	WX_Tool.creatItem=function(_obj,_index,_skin){
		var Item=new Sprite();
		Item.skin=_skin;
		WX_Tool.rankPanel.addChild(Item);
		Item.pos(0,_index *WX_Tool.rankviewMsg.soldH);
		WX_Tool.itemArr.push(Item);
		var ico;
		var score_txt;
		var name_txt;
		var num;
		for(var $each_num in _obj){
			num=_obj[$each_num];
			ico=new Image();
		...
		console.log("————创建",name_txt.text,"名片");
	}
